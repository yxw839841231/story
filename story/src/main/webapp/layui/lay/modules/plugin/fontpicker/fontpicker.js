...
layui.define(['jquery'], function (exports) {
	...
	var defaults = {
			...
			useAttribute      : true,                   // Whether to use attribute selector for printing icons
	...
	Plugin.prototype = {
		...
		renderIconContainer: function () {
			...
			for (var i = 0, item; item = iconsPaged[i++];) {
				...
				// Set the icon box
				$('<span/>', {
					html:      '<i class="layui-icon" data-fip-value="' + item + '" ' + (this.settings.useAttribute ? (this.settings.attributeName + '="' + ( this.settings.convertToHex ? '&#x' + item + ';' : item ) + '"') : 'class=""') + '>&#x' + item + ';</i>',
					'class':   'fip-box',
					title: flipBoxTitle
		...
		setSelectedIcon: function (theIcon) {
			...
			if ( this.settings.useAttribute ) {
				if ( theIcon ) {
					this.iconPicker.find('.selected-icon').html('<i class="layui-icon" ' + this.settings.attributeName + '="' +  theIcon  + '">&#x' + theIcon + ';</i>' );
				} else {
			...
			if ( this.triggerEvent !== null ) {
				...
				for ( var eventKey in this.triggerEvent ) {
					if(typeof this.triggerEvent[eventKey] == 'string') this.element.trigger(this.triggerEvent[eventKey]);
				}
			}
