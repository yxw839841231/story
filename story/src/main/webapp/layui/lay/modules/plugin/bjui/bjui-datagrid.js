...
layui.define([ 'BJUIbasedrag', 'BJUIicheck', 'form'], function (exports) {
    ...
    Datagrid.prototype.TOOLS = function () {
        ...
        var tools = {
            ...
            createTrsByData: function (data, refreshFlag) {
                ...
                if (!that.init_tbody) {
                    ...
                    if (typeof bodyback === 'string') {
                        ...
                        bodyback();
                    } else if (typeof bodyback === 'function') {
                        bodyback();
                    }
                }
            ...
            appendColumns: function () {
                that.childColumn = {
                    name: that.options.keys.gridChild,
                    gridChild: true,
                    width: 30,
                    minWidth: 30,
                    label: '...',
                ...
                that.linenumberColumn = {
                    name: that.options.keys.gridNumber,
                    gridNumber: true,
                    width: 'auto',
                    minWidth: 80,
                    label: 'No.',
                ...
                that.checkboxColumn = {
                    name: that.options.keys.gridCheckbox,
                    gridCheckbox: true,
                    width: 30,
                    minWidth: 30,
                    label: 'Checkbox',
                ...
                that.editBtnsColumn = {
                    name: that.options.keys.gridEdit,
                    gridEdit: true,
                    width: that.options.customEditbtns.width,
                    minWidth: 110,
                    label: (typeof options.showEditbtnscol === 'string' ? options.showEditbtnscol : 'Edit'),
                    align: 'center',
                    menu: false,
                    edit: false,
                    hide: false,
            ...
            initEditInputs: function () {
                ...
                $.each(columnModel, function (i, op) {
                    ...
                    if (op === that.childColumn || op === that.linenumberColumn || op === that.checkboxColumn || op === that.editBtnsColumn) {
                        ...
                        if (op.rule) rule = ' lay-verify="'+ op.label +':'+ op.rule +'"'
                        //if (op.rule) rule = ' lay-verify="' + op.rule + '"'
                        //else if (op.type === 'date') rule = ' lay-verify="pattern('+ (op.pattern || 'yyyy-MM-dd') +')"';
                        if (op.type) {
                            switch (op.type) {
                                case 'date':
                                ...
                                case 'textarea':
                                    ...
                                    break
                                case 'icon':
                                    that.inputs.push('<input type="text" class="zjoin-icon-pick" name="' + name + '"' + rule + attrs + '>')
    ...
    Datagrid.prototype.initTop = function () {
        var that = this, options = that.options, regional = that.regional, hastoolbaritem = false, $group, groupHtml = '<div class="layui-btn-group" role="group"></div>', btnHtml = '<button type="button" class="btn" data-icon=""></button>'
            ...
            if (options.toolbarItem || options.toolbarCustom) {
                if (options.toolbarItem) {
                    ...
                    $.each(options.toolbarItem.split(','), function (i, n) {
                        ...
                        if (n === 'add') {
                            that.$toolbar_add = $(btnHtml).addClass('layui-btn layui-btn-small').html(' <i class="layui-icon">&#xe145;</i>&nbsp;新增')
                                .appendTo($group)
                                .on('click', function (e) {
                                    that.add()
                                })
                        } else if (n === 'edit') {
                            that.$toolbar_edit = $(btnHtml).addClass('layui-btn layui-btn-small').html(' <i class="layui-icon">&#xe254;</i>&nbsp;编辑')
                                .appendTo($group)
                                .on('click', function (e) {
                                    ...
                                    if (!$selectTrs.length) {
                                        ...
                                        layer.msg(BJUI.getRegional('datagrid.selectMsg'), {icon: 5})
                                    } else {
                            ...
                            that.$toolbar_calcel = $(btnHtml).addClass('layui-btn layui-btn-small').html(' <i class="layui-icon">&#xe5c9;</i>&nbsp;取消')
                                .appendTo($group)
                                .on('click', function (e) {
                                    that.doCancelEditRow(that.$tbody.find('> tr.' + that.classnames.tr_edit))
                                })
                        } else if (n === 'save') {
                            that.$toolbar_save = $(btnHtml).addClass('layui-btn layui-btn-small').html(' <i class="layui-icon">&#xe161;</i>&nbsp;保存')
                                .appendTo($group)
                                .on('click', function (e) {
                                    that.doSaveEditRow()
                                })
                        } else if (n === 'del') {
                            that.$toolbar_del = $(btnHtml).addClass('layui-btn layui-btn-small layui-btn-danger').html(' <i class="layui-icon">&#xe65f;</i>&nbsp;删除')
                                .appendTo($group)
                                .on('click', function (e) {
                                    var $selectTrs = that.$tbody.find('> tr.' + that.classnames.tr_selected)
                                    ...
                                    if ($selectTrs.length) {
                                        ...
                                        layer.warn(BJUI.getRegional('datagrid.selectMsg'));
                                    }
                                })
                        } else if (n === 'refresh') {
                            that.$toolbar_refresh = $(btnHtml).addClass('layui-btn layui-btn-small').html(' <i class="layui-icon">&#x1002;</i>&nbsp;刷新')
                                .appendTo($group)
                                .on('click', function (e) {
                                    that.refresh()
                                })
                        } else if (n === 'import') {
                            that.$toolbar_add = $(btnHtml).addClass('layui-btn layui-btn-small').html(' <i class="layui-icon">&#xe0c3;>&nbsp;导入</i>')
                                .appendTo($group)
                                .on('click', function (e) {
                            ...
                            that.$toolbar_add = $(btnHtml).addClass('layui-btn layui-btn-small').html(' <i class="layui-icon">&#xe906;</i>&nbsp;导出')
                                .appendTo($group)
                                .on('click', function (e) {
    ...
    Datagrid.prototype.fixedWidth = function (isInit) {
        ...
        var setNewWidth = function () {
            ...
            var $layerBox = $(".layui-layer-content").length;
            bW = $layerBox > 0 ? (that.$boxB.find('> div'))[0].clientWidth : leftW;
            ...
            if (options.fullGrid || that.isTemplate || that.needfixedWidth || (that.hasAutoCol && isInit)) {
                ...
                if(options.fullGrid) $grid.width(bW + 17).height(that.$boxB.height()).prependTo($('body'))
                else  $grid.width(cW + 17).height(that.$boxB.height()).prependTo($('body'))
    ...
    Datagrid.prototype.filterInThead = function (isDialog) {
        ...
        var doFilter = function (model, val) {
            ...
            tools.quickFilter(model, val ? {operatorA: model.$quickfilter.attr('filter-op'), valA: val} : null)
        }
        ...
        $.each(columnModel, function (i, n) {
            ...
            else if (n.type === 'icon') $input = $('<input type="text" readonly="true" name="' + n.name + '">')
    ...
    Datagrid.prototype.colMenu = function () {
        ...
        $ths.each(function () {
            ...
            //取消表头过滤
            /*  if (!$btnBox.length) {
             $btn    = $('<button type="button" class="btn btn-default"><i class="layui-icon skin-color">&#xe152;</i></button>'),
    ...
    Datagrid.prototype.colFilter = function ($th, filterFlag) {
        ...
        if (model.quickfilter) {
            ...
            if (!$filter || !$filter.length) {
                ...
                $input.removeAttr('lay-verify').attr('size', 10).addClass('filter-input')
    ...
    Datagrid.prototype.initPaging = function () {
        ...
        pagingHtml
            ...
            .add('<span></span><div class="paging-pagesize"><button type="button" class="btn-default btn-refresh" title="刷新" data-icon="1002;"></button>')
        ...
        var doDel = function () {
            ...
            if (options.delUrl) {
                ...
                var type = options.delType, opts = {
                    ...
                    type: 'POST',
                    okCallback: callback || function (json) {
                        delEnd()
                    }
                }
...
// Specification post data
        ...
        if (!len) {
            ...
            if (typeof back === 'string') {
                ...
                if (that.tools.beforeSave($trs, datas)) {
                    var type = options.editType, opts = {
                        url: options.editUrl,
